...
interface HomebodyProps {
  tracks: HomeTrack[];
  onMenuChange: (key: any, searchTerm?: string) => void;
}
...
const Homebody: React.FC<HomebodyProps> = ({ tracks, onMenuChange }) => {
  ...
  const stats = useMemo(() => [
    { title: "Regions", attr: "region", data: Array.from(new Set(approvedTracks.map((t) => t.region))).filter(Boolean) as string[] },
    { title: "Countries", attr: "country", data: Array.from(new Set(approvedTracks.map((t) => t.country))).filter(Boolean) as string[] },
    { title: "Categories", attr: "category", data: Array.from(new Set(approvedTracks.map((t) => t.category))).filter(Boolean) as string[] },
    { title: "Communities", attr: "community", data: Array.from(new Set(approvedTracks.map((t) => t.community))).filter(Boolean) as string[] }
  ], [approvedTracks]);
          ...
          {activeDetail && (
            <div className="absolute top-4 left-0 right-0 p-6 bg-white/95 backdrop-blur-md rounded-2xl border border-gray-100 shadow-2xl z-40 max-w-2xl mx-auto lg:mx-0">
              <div className="flex justify-between items-center mb-4 border-b pb-2">
                <h4 className="text-xs font-bold text-[#E67E22] uppercase tracking-widest">Select from {activeDetail.title}</h4>
              ...
              <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {activeDetail.items.map((item, idx) => (
                  <button 
                    ...
                    className="px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-full text-[10px] font-bold text-gray-600 uppercase hover:bg-orange-500 hover:text-white hover:border-orange-500 transition-all"
                  >
                    {item}
                  </button>
                ))}
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {featuredTracks.map((track) => {
              ...
              return (
                <div 
                  key={trackId} 
                  className="bg-white flex flex-col border-x-2 border-b-2 relative shadow-sm transition-transform hover:scale-[0.97] flex-shrink-0 w-[240px]" 
