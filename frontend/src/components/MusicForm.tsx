                ...
                <div className="space-y-4">
                  ...
                  {failedSummary.map((f, i) => (
                    <div key={i} className="bg-red-50 p-2 sm:p-3 rounded-xl border border-red-100 flex flex-col gap-2">
                      ...
                      {f.similarTrack && <div className="mt-1 pt-2 border-t border-red-200 flex items-center gap-3"><img src={f.similarTrack.album_file_url} className="w-10 h-10 rounded-full object-cover" onError={(e) => e.currentTarget.src = "/placeholder-image.png"} /><div className="flex flex-col max-w-[90px]"><p className="text-[10px] font-bold text-gray-800 truncate">{f.similarTrack.title}</p><p className="text-[9px] text-gray-600 truncate">{f.similarTrack.performer}</p></div><audio controls className="flex-grow h-7 scale-90 origin-right" src={f.similarTrack.sound_track_url} /></div>}
                    </div>
                  ))}
                </div>
              )}
