...
const MusicForm: React.FC<MusicFormProps> = ({ onTrackAdded, onTrackUpdated, onCancelEdit, editingTrack }) => {
  ...
  const handleFieldChange = (fieldName: string, value: string) => {
    ...
    let error = "";
    if (value.trim() === "" && fieldName !== "description" && fieldName !== "contributor") {
      error = `${toSentenceCase(fieldName)} is required.`;
    } else if (restrictedFields.includes(fieldName) && !validateChars(value)) {
  ...
  const processSingleSubmission = async (data: any, audioFile: any, coverFile: any, index?: number, isEdit: boolean = false) => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const response = isEdit && editingTrack 
      ...
      apiResult = {
        success: false,
        error: String(err.response?.data?.error || err.message),
        step: err.response?.data?.step || "sync",
        similarTrack: err.response?.data?.similarTrack || null
      };
    }

    const steps = ["duration", "loudness", "similarity", "sync"];
    const checkKeys: (keyof CheckListState)[] = ["lengthCheck", "audibilityCheck", "fingerprintCheck", "syncCheck"];

    for (let i = 0; i < steps.length; i++) {
      const currentStep = steps[i];
      const currentKey = checkKeys[i];

      if (i > 0) {
        setCheckList(prev => ({ ...prev, [currentKey]: { status: "loading", error: "" } }));
        await delay(1500);
      }

      if (!apiResult.success && apiResult.step === currentStep) {
        setCheckList(prev => ({ ...prev, [currentKey]: { status: "fail", error: apiResult.error || "" } }));
        setIsLoading(false);
        return apiResult;
      } else {
        setCheckList(prev => ({ ...prev, [currentKey]: { status: "pass", error: "" } }));
      }
    }
