...
interface Props { tracks: Track[]; onEdit: (track: Track) => void; onRefresh?: () => void; }

const MusicList: React.FC<Props> = ({ tracks, onEdit, onRefresh }) => {
  ...
  const isStandardUser = role !== "admin" && role !== "superadmin";
  ...
  const renderTrackCard = (t: Track) => {
    ...
    const isContributor = userEmail && t.contributor === userEmail;
      ...
      <div key={t.sound_id} className={`bg-white flex flex-col border-x border-b relative shadow-sm w-[220px] mx-auto sm:mx-0 ${isPending ? "border-orange-500" : "border-gray-200"}`} style={{ borderRadius: "1000px 1000px 16px 16px", height: 'fit-content' }}>
        ...
        <div className="px-3 py-3 flex flex-col text-center">
          <div className="flex items-center justify-center gap-1 mb-0">
            ...
            {isContributor && <div className="text-blue-600 shrink-0" title="your uploads"><svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg></div>}
          </div>
          ...
          <div className="mt-2 flex justify-between items-center border-t border-gray-100 pt-2">
            <div className="flex flex-wrap gap-2">
              {(role === "admin" || role === "superadmin") && <button onClick={() => onEdit(t)} className="text-blue-700 text-[11px] font-semibold hover:underline">Edit</button>}
              {(role === "admin" || role === "superadmin") && (isPending ? (<><button onClick={() => setModal({ show: true, id: t.sound_id, title: t.title, type: "approve" })} className="text-orange-700 text-[11px] font-semibold hover:underline">Approve</button> <button onClick={() => setModal({ show: true, id: t.sound_id, title: t.title, type: "reject" })} className="text-red-700 text-[11px] font-semibold hover:underline">Reject</button></>) : <button onClick={() => setModal({ show: true, id: t.sound_id, title: t.title, type: "unapprove" })} className="text-red-700 text-[11px] font-semibold hover:underline">Remove</button>)}
            </div>
            <button onClick={() => setExpandedId(isExp ? null : t.sound_id)} className="text-indigo-700 text-[11px] font-semibold underline">{isExp ? "Less" : "Details"}</button>
          </div>
          {isExp && (
  ...
  return (
    <div className="w-full">
      ...
      {isStandardUser && myPendingTracks.length > 0 && (
        <div className="mt-12">
