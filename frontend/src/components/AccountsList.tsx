...
interface UserAccount { id: number; name: string; email: string; country: string; role: "superadmin" | "admin" | "user" | "deactivated"; email_verified: boolean; }
...
const AccountsList: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden text-slate-600">
          ...
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-[2] py-2.5 rounded-xl text-[9px] font-bold bg-orange-50 text-[#E67E22] border border-orange-100 shadow-sm flex items-center justify-center transition-all hover:bg-orange-100"
              >
                {isProcessing ? <div className="h-3 w-3 border-2 border-[#E67E22]/30 border-t-[#E67E22] rounded-full animate-spin" /> : "Confirm change"}
              </button>
      ...
      <div className="px-6 py-4 border-b bg-gray-50/30 flex justify-center items-center">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-50">
            {users.map((u, index) => (
              <tr 
                key={u.id} 
                className={`transition-all duration-200 ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-orange-50/30`}
              >
                <td className="px-6 py-3 text-[11px] lowercase">{u.name}</td>
                <td className="px-6 py-3 text-[11px] lowercase text-gray-500">{u.email}</td>
                <td className="px-6 py-3 text-[11px] lowercase text-gray-500">{u.country}</td>
                ...
                {isSuperAdmin && (
                  <td className="px-6 py-3 text-center">
                    <select 
                      value={u.role} 
                      onChange={e => setModal({ show: true, id: u.id, name: u.name, role: e.target.value })} 
                      className="text-[10px] border-gray-200 border px-2 py-1 rounded-lg bg-white/50 focus:bg-white cursor-pointer outline-none transition-all hover:border-orange-200"
                    >
