...
const ModernMusicList: React.FC<Props> = ({ 
  tracks, 
  onEdit, 
  onRefresh, 
  userRole, 
      ...
      {confirmModal.show && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-gray-100">
            ...
            <p className="text-gray-500 text-sm mb-6">Confirm action for {confirmModal.title}</p>
            <div className="flex gap-3">
              <button 
                disabled={isProcessing}
                onClick={handleAction} 
                className={`flex-[2] px-4 py-2 rounded-xl text-xs font-bold transition-all border flex items-center justify-center gap-2 ${
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    {isContributor && (
                       <span className="text-[9px] text-gray-400 italic bg-gray-50 px-1 rounded">Your upload</span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    {isAdmin && (
                      isPending ? (
                        <>
                          <button 
                            onClick={() => setConfirmModal({ show: true, trackId: trackId, title: track.category || "Modern", type: "approve" })}
                            className="text-[#E67E22] text-[11px] font-bold hover:underline"
                          >
                            Approve
                          </button>
                          <button 
                            onClick={() => setConfirmModal({ show: true, trackId: trackId, title: track.category || "Modern", type: "reject" })}
                        ...
                        <button 
                          onClick={() => setConfirmModal({ show: true, trackId: trackId, title: track.category || "Modern", type: "unapprove" })}
                          className="text-[#E67E22] text-[11px] font-bold hover:underline"
                        >
                          Remove
                        </button>
