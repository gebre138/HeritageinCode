...
import { Music, Volume2, Mic2, RefreshCw, Loader2, AlertCircle, TriangleAlert, Upload, SlidersHorizontal} from "lucide-react";
...
const MusicFusion: React.FC<Props> = ({ tracks, modernTracks: initialModernTracks, initialTrack }) => {
  ...
  const handleSliderChange = async (val: number, isMelody: boolean) => {
    ...
    if (isMelody) {
      ...
      if (filter1.current) {
        filter1.current.gain.setTargetAtTime((val - 50) * 0.4, audioCtx.current!.currentTime, 0.1);
      }
    } else {
      setStyleStrength(val);
      if (filter2.current) {
        filter2.current.gain.setTargetAtTime((val - 50) * 0.4, audioCtx.current!.currentTime, 0.1);
      }
    }
  };

  useEffect(() => {
    if (music1 && audioRef1.current) initAudio(audioRef1, filter1, source1);
  }, [music1]);

  useEffect(() => {
    if ((music2 || userModernUrl) && audioRef2.current) initAudio(audioRef2, filter2, source2);
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/modern/jamendo`)
      .then(res => res.json())
      .then(data => setJamendoTracks(data.map((t: any) => ({ ...t, isapproved: true }))))
      .catch(console.error);
    fetchPricing();
  ...
  useEffect(() => {
    if (initialTrack) {
      setMusic1(initialTrack);
      setSearch1(initialTrack.title.toLowerCase());
    }
  }, [initialTrack]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setUserModernFile(file);
      setUserModernUrl(URL.createObjectURL(file));
      setMusic2(null);
      setSearch2("");
    }
  };
  ...
  const handleFusion = async () => {
    ...
    try {
      ...
      storageFd.append("modern_sound", fusionMode === "audio" ? (music2?.title || "modern") : "user upload");
      ...
      {showError && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm" style={{ backgroundColor: COLORS.bgBlackOverlay }}>
          <div className="rounded-3xl shadow-2xl max-w-sm w-full p-6 text-center space-y-6 animate-shake" style={{ backgroundColor: COLORS.bgWhite }}>
            ...
            <div className="space-y-2"><p className="text-[13px] font-normal" style={{ color: COLORS.textLight }}>fusion failed, try again later</p></div>
        ...
        <div ref={containerRef1} className="space-y-4">
          <div className="p-4 rounded-2xl h-[100px] relative border" style={{ backgroundColor: COLORS.bgWhite, borderColor: COLORS.borderLight }}>
            ...
            <div className="relative mt-2">
              ...
              {isOpen1 && <div className="absolute z-30 w-full mt-1 border rounded-xl max-h-40 overflow-auto shadow-xl bg-white">{fH.map(t => <button key={t.sound_id} className="w-full text-left p-2 text-[13px] border-b last:border-0 hover:bg-gray-50" onClick={() => {setMusic1(t); setIsOpen1(false); setSearch1(t.title.toLowerCase());}}>{t.title.toLowerCase()}</button>)}</div>}
            </div>
          </div>
          {music1 && (
            ...
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2 items-center">
                <div className="flex items-center gap-1">
                  <span className="text-[9px] font-bold opacity-50">Use existing library</span>
                  <button onClick={() => setFusionMode("audio")} className={`p-1.5 rounded-lg transition-all ${fusionMode === "audio" ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-400"}`}><Music size={14} /></button>
                </div>
                <div className="flex items-center gap-1">
                  <span className="text-[9px] font-bold opacity-50">Upload your track</span>
                  <button onClick={() => { setFusionMode("upload"); fileInputRef.current?.click(); }} className={`p-1.5 rounded-lg transition-all ${fusionMode === "upload" ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-400"}`}><Upload size={14} /></button>
                </div>
              </div>
            </div>
            {fusionMode === "upload" ? (
          ...
          <div className="w-full flex items-center gap-4 p-4 rounded-2xl border animate-in slide-in-from-bottom-2 bg-white" style={{ borderColor: COLORS.borderLight }}>
            ...
            <div className="flex flex-col items-center">
              <TransactionManager 
                ...
                currentUserEmail={userEmail} downloadUrl={url} onOpenLogin={() => setLoginModal(true)} price={fusionPrice} variant="fused"
              />
