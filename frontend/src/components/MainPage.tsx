...
const MainPage: React.FC = () => {
  ...
  const getFilteredList = (list: Track[]) => list.filter(t => {
    ...
    if (searchAttrs.includes("all")) {
        const searchableKeys = libraryType === "modern" 
          ? ["country", "category"] 
          : ["title", "performer", "country", "category"];
          
        return searchableKeys.some(k => 
            (t as any)[k]?.toString().toLowerCase().includes(term)
        );
    }
    return searchAttrs.some(attr => (t as any)[attr]?.toString().toLowerCase().includes(term));
  });
  ...
  const handleAttrToggle = (attr: string) => {
    ...
    setSearchAttrs(prev => {
      ...
      if (filtered.includes(attr)) {
        const next = filtered.filter(a => a !== attr);
        return next.length === 0 ? ["all"] : next;
      }
      return [...filtered, attr];
    });
    setShowDataDropdown(prev => (prev === attr ? null : attr));
  };

  const getAttributeData = (attr: string) => {
    const list = libraryType === "traditional" ? tracks : modernTracks;
    const filteredByStatus = list.filter(t => 
        (userRole === "admin" || userRole === "superadmin") 
            ? (!!t.isapproved === (approvalStatus === "approved")) 
            : !!t.isapproved
    );
    return Array.from(new Set(filteredByStatus.map((t: any) => t[attr]))).filter(Boolean).sort();
  };

  const handleDataSelect = (attr: string, value: string) => {
    setSearchAttrs(["all"]);
    setSearchTerm(`${toSentenceCase(attr)}: ${value}`);
    setShowDataDropdown(null);
  };
  ...
  const filterOptions = libraryType === "modern" 
    ? ["all", "country", "category"] 
    : ["all", "title", "performer", "country", "category"];
                ...
                <div className="bg-[#FFFBEB] rounded-2xl p-4 mb-8 border border-orange-100 shadow-sm space-y-4">
                  {(userRole === "admin" || userRole === "superadmin") && (
                    <div className="relative flex p-1 bg-gray-100/50 rounded-xl border w-full max-md mx-auto">
                      ...
                      <button className={`relative flex-1 py-2.5 text-sm font-bold ${approvalStatus === "approved" ? "text-[#E67E22]" : "text-gray-500"}`} onClick={() => { setApprovalStatus("approved"); setSearchTerm(""); }}>Approved sounds</button>
                      <button className={`relative flex-1 py-2.5 text-sm font-bold flex items-center justify-center gap-2 ${approvalStatus === "pending" ? "text-[#E67E22]" : "text-gray-500"}`} onClick={() => { setApprovalStatus("pending"); setSearchTerm(""); }}>
                        Pending 
                  ...
                  <div className="flex flex-col md:flex-row bg-white rounded-xl border border-orange-200 shadow-sm overflow-visible relative">
                    <div className="relative flex-grow flex items-center min-h-[56px] border-b md:border-b-0 md:border-r border-orange-100 py-2">
                      ...
                      <div className="flex flex-wrap items-center pl-12 pr-4 gap-2 w-full">
                        {searchAttrs.filter(a => a !== "all").map(attr => (
                          <button 
                            key={attr} 
                            onClick={() => handleAttrToggle(attr)}
                            className="bg-orange-500 text-white text-[12px] font-medium px-3 py-1.5 rounded-full flex items-center gap-2 shadow-sm hover:bg-orange-600 transition-all animate-in zoom-in-90"
                          >
                            {toSentenceCase(attr)}
                        ...
                        <input 
                          type="text" 
                          placeholder={searchAttrs.includes("all") ? `Search ${libraryType} tracks...` : `Type to search...`} 
                          className="flex-grow min-w-[120px] outline-none font-normal text-sm bg-transparent h-10" 
                          value={searchTerm} 
                          onChange={(e) => setSearchTerm(e.target.value)} 
                        />
                        {searchTerm && (
                          <button onClick={() => setSearchTerm("")} className="text-gray-400 hover:text-orange-500 text-xs px-2">Clear</button>
                        )}
                      </div>
                    </div>

                    <div className="flex items-center px-4 bg-orange-50/20 min-h-[56px]">
                      <span className="text-[10px] font-bold uppercase text-orange-400 mr-4">Filter:</span>
                      <div className="flex gap-2 flex-wrap py-2">
                        {filterOptions.map((opt, idx, arr) => (
                          <div key={opt} className="relative">
                            <button
                              onClick={() => handleAttrToggle(opt)}
                              className={`text-[11px] font-medium px-3 py-1.5 rounded-lg transition-all border ${searchAttrs.includes(opt) ? "bg-orange-500 text-white border-orange-500 shadow-sm" : "bg-white text-gray-500 border-gray-200 hover:border-orange-300"}`}
                            >
                              {toSentenceCase(opt)}
                            </button>
                            {showDataDropdown === opt && opt !== "all" && (
                              <div ref={dataDropdownRef} className={`absolute ${idx === arr.length - 1 ? 'right-0' : 'left-0'} top-full mt-3 w-48 bg-white border border-orange-200 shadow-2xl rounded-xl z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-150`}>
                                <div className="max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-orange-200">
                                  {getAttributeData(opt).map((val) => (
                                    <button
                                      key={val}
                                      onClick={() => handleDataSelect(opt, val)}
                                      className="w-full text-left px-4 py-2.5 text-xs text-gray-700 hover:bg-orange-500 hover:text-white transition-colors border-b border-gray-50 last:border-0 truncate"
                                    >
                                      {val}
                                    </button>
                                  ))}
