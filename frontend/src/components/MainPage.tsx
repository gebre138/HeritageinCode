...
const MainPage: React.FC = () => {
  const [activeMenu, setActiveMenu] = useState<MenuKey | "balance" | "guide" | "identify">("home");
            ...
            <nav className="hidden lg:flex space-x-6 items-center">
              {visibleMenus.map((m: any) => (
                <div key={m.key} className="relative group" onMouseEnter={() => setDropdowns(p => ({ ...p, [m.key]: true }))} onMouseLeave={() => setDropdowns(p => ({ ...p, [m.key]: false }))}>
                  <button onClick={() => handleMenuClick(m.key)} className="py-2 px-1 flex items-center whitespace-nowrap" style={{ color: (activeMenu === m.key || (m.key === 'library' && activeMenu === 'statistics') || (m.key === 'learn' && (activeMenu === 'identify' || activeMenu === 'guide'))) ? COLORS.primaryColor : 'inherit' }}>
                  ...
                  {m.key === "library" && dropdowns.library && (
                    <div className="absolute left-0 w-56 bg-white border shadow-2xl rounded-lg py-2 z-[600]" style={{ borderColor: COLORS.borderLight }}>
                      ...
                      <button onClick={() => selectLibraryType("modern")} className="w-full text-left px-4 py-2 text-sm hover:bg-orange-50 flex items-center gap-2">
                        <span>Modern</span>
                        {(userRole === "admin" || userRole === "superadmin") && pendingModernCount > 0 && (
                  ...
                  {m.key === "learn" && dropdowns.learn && (
                    <div className="absolute left-0 w-56 bg-white border shadow-2xl rounded-lg py-2 z-[600]" style={{ borderColor: COLORS.borderLight }}>
                      ...
                      <button onClick={() => { setActiveMenu("guide"); setDropdowns(p => ({ ...p, learn: false })); }} className="w-full text-left px-4 py-2 text-sm hover:bg-orange-50">Rhythm guide</button>
                    </div>
                  )}
                </div>
              ))}
              ...
              <div className="flex flex-col py-2">
                {visibleMenus.map((m: any, idx) => (
                  <div key={m.key} className="flex flex-col" style={{ backgroundColor: idx % 2 === 0 ? COLORS.lightColor : COLORS.bgWhite }}>
                    <button onClick={() => handleMenuClick(m.key)} className="flex justify-between items-center py-3 px-4" style={{ color: (activeMenu === m.key || (m.key === 'library' && activeMenu === 'statistics') || (m.key === 'learn' && (activeMenu === 'identify' || activeMenu === 'guide'))) ? COLORS.primaryColor : COLORS.textDark }}>
                    ...
                    {m.key === 'learn' && mobileOptions === 'learn' && (
                      <div className="flex flex-col border-l-2" style={{ borderColor: COLORS.primaryColor, backgroundColor: COLORS.bgLibrary }}>
                        ...
                        <button onClick={() => { setActiveMenu("guide"); setIsMenuOpen(false); }} className="text-left px-6 py-2 text-[13px]">Rhythm guide</button>
                      </div>
                    )}
                  </div>
                ))}
      ...
      <main className="flex-grow">
        {activeMenu === "home" ? (
          ...
          <div className="max-w-7xl mx-auto px-6 py-10 relative z-10">
            ...
            {activeMenu === "guide" && <InstrumentGuide tracks={tracks.filter(t => t.isapproved)} />}
            {activeMenu === "balance" && (
              <BalanceDashboard 
                currentUser={{ 
